...
//	http://catalogapi.azure.com/offers/?api-version=2018-08-01-beta
const _data_file = "az-mp-api-dump.js";
const _clean_file = "az-mp-api-clean.js";
...
function OpenAzureMarketPlaceFile(_d_file, _c_file)
{
	Console.log("OpenAzureMarketPlaceFile", _d_file, _c_file);

	let _new_data = JSON.parese(filesystem.readFileSync(_d_file, "utf8"));
	//	Console.log("_new_data", _new_data);




	// write out the cleaned JSON object
	var stream = filesystem.createWriteStream( _c_file,
		{
			flags: 'w',
			defaultEncoding: 'utf8',
			fd: null,
	...
	stream.once( 'open', function ( fd )
	{
		stream.write( _new_data );
		stream.close();
		stream.end();
	} );

	Console.log( "end:_writeToFile::", _c_file );
	return;
}

function ScrubMarketPlaceDataFile()
{
	Console.log( "BEGIN:WriteSvgAssetsFiles" );
	//	write out a local version of this file
	OpenAzureMarketPlaceFile(_data_file, _clean_file);

	Console.log( "END:WriteSvgAssetsFiles" );
	return;
}

ScrubMarketPlaceDataFile();